...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("You've already signup, please login")
        else:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user_log:
            if check_password_hash(user_log.password, form.password.data):
                login_user(user_log)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            new_comment = Comment(text=form.comment.data, blog_id=post_id, comment_author=current_user,
                                  author_id=current_user.id, email=current_user.email)
